...
class asmrgay {
  /** Alist API 基础 URL */
  static apiBase = 'https://www.asmrgay.com';

  /** 精选歌单根目录路径 */
  static FEATURED_DIR = '/asmr/中文音声';
            ...
            if (searchType === '1') {
              ...
              result = content
                ...
                .map((f) => {
                  ...
                  return {
                    id: `ag${encodeURIComponent(`${parentDir}/${f.name}`)}`,
                    title: f.name.replace(/\.[^.]+$/, ''),
                    artist: '',
                    artist_id: '',
                    album: dirName,
                    album_id: `agdir_${encodeURIComponent(parentDir)}`,
                    source: 'asmrgay',
  ...
  static _list_dir(url) {
    const listId = getParameterByName('list_id', url);
    const dirPath = decodeURIComponent(listId.replace(/^agdir_/, ''));
        ...
        axios
          ...
          .then((response) => {
            ...
            const tracks = content
              .filter((f) => !f.is_dir && this._isAudioFile(f.name))
              .map((f) => ({
                id: `ag${encodeURIComponent(`${dirPath}/${f.name}`)}`,
                title: f.name.replace(/\.[^.]+$/, ''),
                artist: '',
                artist_id: '',
                album: dirPath
