# AGENTS.md — Listen1 Chrome Extension
...
Provider ID 映射：`ne`=网易、`qq`=QQ、`kg`=酷狗、`kw`=酷我、`bi`=B站、`mg`=咪咕、`th`=千千、`gn`=GD网易、`jx`=Joox、`lm`=本地、`my`=我的歌单、`xm`=虾米。
...
```js
/* global l1Player MediaService */
/* global netease qq kugou kuwo bilibili migu taihe gdnetease joox localmusic myplaylist */
```
